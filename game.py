...
class Game:
    ...
    def __init__(self) -> None:
        ...
        self._screen_flash_for = 0.0
        self._hurt_text_for = 0.0
    ...
    def _cue_hit(self, source_rect: pygame.Rect) -> None:
        self._screen_flash_for = max(self._screen_flash_for, 0.12)
        self._hurt_text_for = max(self._hurt_text_for, 0.35)
    ...
    def update(self, dt: float) -> None:
        ...
        if self._screen_flash_for > 0:
            self._screen_flash_for = max(0.0, self._screen_flash_for - dt)

        if self._hurt_text_for > 0:
            self._hurt_text_for = max(0.0, self._hurt_text_for - dt)
    ...
    def draw(self) -> None:
        ...
        if self._hurt_text_for > 0:
            text = self.big_font.render("HURT!", True, self.palette.hazard)
        ...
        if self._screen_flash_for > 0:
            a = _clamp(self._screen_flash_for / 0.12, 0.0, 1.0)
            overlay = pygame.Surface((self.SCREEN_W, self.SCREEN_H), pygame.SRCALPHA)
            overlay.fill((255, 255, 255, int(140 * a)))
...
def _make_player_anims(color: pygame.Color) -> dict[str, Animation]:
    ...
    return {
        ...
        "run": Animation(run_frames, fps=14.0),
...
def _draw_player_frame(color: pygame.Color, *, leg_phase: int, eye_open: bool) -> pygame.Surface:
    ...
    dx = 6
    phase = leg_phase % 4
    left_off = (-dx, 8) if phase in {0, 3} else (-dx, 1)
    right_off = (dx, 8) if phase in {1, 2} else (dx, 1)

    pygame.draw.line(surf, pygame.Color("#EEB4CC"), (w // 2 - 6, leg_y), (w // 2 - 6 + left_off[0] // 3, leg_y + left_off[1]), 4)
    pygame.draw.line(surf, pygame.Color("#EEB4CC"), (w // 2 + 6, leg_y), (w // 2 + 6 + right_off[0] // 3, leg_y + right_off[1]), 4)

    # Arms
    arm_y = body.top + 10
    arm_wiggle = -3 if phase in {0,2} else 3
    pygame.draw.line(surf, pygame.Color("#EEB4CC"), (body.left + 3, arm_y), (body.left - 6, arm_y + arm_wiggle), 4)
    pygame.draw.line(surf, pygame.Color("#EEB4CC"), (body.right - 3, arm_y), (body.right + 6, arm_y + arm_wiggle), 4)
